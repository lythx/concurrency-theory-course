...
\[
    \begin{aligned}
        ...
        \Sigma_B & = \{ B_{i,j,p} \mid i \in I,\ j \in J,\ j \ge i \ \},                                          \\
        \Sigma_C & = \{ C_{i,k} \mid i \in I,\ k \in I,\ k \ne i \},                                              \\
        \Sigma_E & = \{ D_{i,j,k} \mid i \in I,\ j \in J,\ k \in I,\ j \ge i,\ k \ne i \},                        \\
        \Sigma_F & = \{ E_{i,j} \mid i \in I,\ j \in J,\ j \ge i \},                                              \\
        \Sigma_G & = \{ F_{i,j,k} \mid i \in I,\ j \in J,\ k \in I,\ j \ge i,\ k \ne i \}.
    \end{aligned}
\]
...
\[
    \begin{aligned}
        D^{\uparrow}_i = & ~\{ (C_{i,k}, D_{i,j,k})   \mid j \in J_{\ge i},~k \in K_{< i} \}  \\
        \cup             & ~\{ (D_{i,j,k}, F_{i,j,k}) \mid j \in J_{\ge i},~k \in K_{< i} \}.
    \end{aligned}
\]
...
\[
    D^{\uparrow}_{i,i-1} = \{ (F_{i,j,k}, C_{i-1,k}) \mid j \in J_{\ge i},~k \in K_{< i} \}
\]
...
\[
    D = \operatorname{sym}\Biggl(\Bigl\{\Bigl(\bigcup_{i=0}^{n-2} D^{\downarrow}_i\Bigr) \cup \Bigl(\bigcup_{i=0}^{n-2} D^{\downarrow}_{i,i+1}\Bigr) \cup \Bigl(\bigcup_{i=1}^{n-1} D^{\uparrow}_i\Bigr) \cup \Bigl(\bigcup_{i=2}^{n-1} D^{\uparrow}_{i,i-1}\Bigr)\Bigr\}^{+}\Biggr) \cup I_\Sigma,
\]
...
\[
    V = \Sigma,
    \quad
    E = \Bigl(\bigcup_{i=0}^{n-2} D^{\downarrow}_i\Bigr) \cup \Bigl(\bigcup_{i=0}^{n-2} D^{\downarrow}_{i,i+1}\Bigr) \cup \Bigl(\bigcup_{i=1}^{n-1} D^{\uparrow}_i\Bigr) \cup \Bigl(\bigcup_{i=2}^{n-1} D^{\uparrow}_{i,i-1}\Bigr)
\]
...
\[
    \begin{aligned}
        ...
        \mathcal{E}_{n-1}           & = \{ E_{n-1,j} \mid j \in J_{\ge n-1} \},                                                                          \\
        \mathcal{C}_i^{\uparrow}    & = \{ C_{i,k} \mid k \in K_{< i} \},                                                             & 1 \le i \le n-1, \\
        \mathcal{D}_i^{\uparrow}    & = \{ D_{i,j,k} \mid j \in J_{\ge i},\ k \in K_{< i} \},                                         & 1 \le i \le n-1, \\
        \mathcal{F}_i^{\uparrow}    & = \{ F_{i,j,k} \mid j \in J_{\ge i},\ k \in K_{< i} \},                                         & 1 \le i \le n-1.
    \end{aligned}
\]
...
Funkcja \texttt{gauss\_jordan\_cuda} w pliku \texttt{gauss\_jordan.cu} utrzymuje macierz w pamięci GPU i za pomocą kerneli z pliku \texttt{kernels.cu} realizuje kolejne warstwy Foaty. Wykorzystuje przy tym dodatkowy wektor na mnożniki eliminacji oraz macierz na składniki do odejmowania. Znajdywanie pivotów odbywa się na CPU.
